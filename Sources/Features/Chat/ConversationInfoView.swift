        ...
        VStack(spacing: 10) {
            ...
            VStack(spacing: 0) {
                ...
                if let seen = conversation.metadata?.lastSeenAt, let date = parseISODate(seen) {
                    infoRow("آخر ظهور", seenText(date))
                }
                if let site = conversation.metadata?.website, !site.isEmpty {
                    HStack {
                        Text("الموقع").font(.system(size: 13)).foregroundStyle(Theme.onMuted)
                        Spacer()
            ...
            if let labels = conversation.metadata?.labels, !labels.isEmpty {
                VStack(alignment: .leading, spacing: 8) {
                    Text("التصنيفات").font(.system(size: 13, weight: .semibold)).foregroundStyle(Theme.onMuted)
